...
async def ban_handler(client: Client, msg: types.Message):
    ...
    if not await db.cek_user_didatabase():
        return await msg.reply_text(
            text=f"<i>{get_user_mention(target, client)}</i> tidak terdaftar didatabase",
            quote=True,
            parse_mode=enums.ParseMode.HTML
        )
    status = [
    ...
    if member.status == 'admin' in status:
        return await msg.reply_text(
            text=f"❌<i>Terjadi kesalahan, {get_user_mention(target, client)} adalah seorang {member.status.upper()} tidak dapat dibanned</i>",
            quote=True,
            parse_mode=enums.ParseMode.HTML
        )
    ...
    notification_text = f"User {get_user_mention(target, client)} dengan Id: {target} sudah di banned. karena: {alasan}"
    await client.send_message(config.channel_1, notification_text)

    admin_mention = get_user_mention(config.id_admin, client)
    return await msg.reply_text(
        text=f"{get_user_mention(target, client)} <i>berhasil dibanned</i>\n└Dibanned oleh : {admin_mention}\n\nAlasan: {str(alasan)}\n\n{update}",
        quote=True,
        parse_mode=enums.ParseMode.HTML
    )
...
async def unban_handler(client: Client, msg: types.Message):
    ...
    if await db.cek_user_didatabase():
        if target in db.get_data_bot(client.id_bot).ban:
            ...
            # Send notification to channel_1
            notification_text = f"User {get_user_mention(target, client)} dengan Id: {target} telah unbanned. Alasan: {alasan}"
            await client.send_message(config.channel_1, notification_text)

            admin_mention = get_user_mention(config.id_admin, client)
            return await msg.reply_text(
                text=f"{get_user_mention(target, client)} <i>berhasil diunbanned</i>\n└Diunbanned oleh : {admin_mention}\n\nAlasan: {str(alasan)}",
                quote=True,
                parse_mode=enums.ParseMode.HTML
            )
        else:
            return await msg.reply_text(
                text=f"<i>{get_user_mention(target, client)}</i> sedang tidak dalam kondisi banned",
                quote=True,
                parse_mode=enums.ParseMode.HTML
            )
    else:
...
def get_user_mention(user_id: str, client: Client):
    user_name = get_mention_name(user_id, client)  # Assuming the 'client' object is available here

    return f"<a href='tg://user?id={str(user_id)}'>{user_name}</a>"
