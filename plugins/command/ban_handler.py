...
async def ban_handler(client: Client, msg: types.Message):
    ...
    if not await db.cek_user_didatabase():
        return await msg.reply_text(
            text=f"<i>{get_mention_name(target)}</i> tidak terdaftar didatabase", quote=True,
            parse_mode=enums.ParseMode.HTML
        )
    status = [
    ...
    if member.status == 'admin' in status:
        return await msg.reply_text(
            text=f"❌<i>Terjadi kesalahan, {get_mention_name(target)} adalah seorang {member.status.upper()} tidak dapat dibanned</i>", quote=True,
            parse_mode=enums.ParseMode.HTML
        )
    ...
    notification_text = f"User {get_mention_name(target)} dengan Id: {target} sudah di banned. karena: {alasan}"
...
async def unban_handler(client: Client, msg: types.Message):
    ...
    if await db.cek_user_didatabase():
        if target in db.get_data_bot(client.id_bot).ban:
            ...
            # Send notification to channel_1
            notification_text = f"User {get_mention_name(target)} dengan Id: {target} telah unbanned. Alasan: {alasan}"
            await client.send_message(config.channel_1, notification_text)
            ...
            return await msg.reply_text(
                text=f"<i>{get_mention_name(target)}</i> sedang tidak dalam kondisi banned", quote=True,
                parse_mode=enums.ParseMode.HTML
            )
    else:
        return await msg.reply_text(
...
async def get_mention_name(user_id: str):
    # Get user information
